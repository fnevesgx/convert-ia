...
| Spec com issue aberta — `status` da spec fica congelado em `aprovado` de propósito, o tracker é quem manda daqui pra frente | Execução (dev / code review / QA) | Antes do QA humano, testes a partir das regras da seção 6 no sistema novo | [`characterization-tester`](../characterization-tester/SKILL.md) |
...
- Não interpreta pedido ambíguo de lote ("todas as specs") como escopo fechado — a skill apontada deve reconfirmar ids/cortes antes de gravar em massa.
